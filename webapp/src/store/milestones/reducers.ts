...
export function reducer(
  state: State = milestonesInitialState,
  action: Actions
) {
  switch (action.type) {
    case ActionTypes.CREATE_MILESTONE: {
      const ms = action.payload;
      const ll = sortMilestones(
        filterMilestonesOnProject(state.items, ms.projectId)
      );
      ms.rank =
        ll.length === 0
          ? Rank("", "").rank
          : Rank(ll[ll.length - 1].rank, "").rank;
      return {
        ...state,
    ...
    case ActionTypes.LOAD_MILESTONES: {
      const milestones = action.payload.map(
        (x) => ({ ...x, kind: "milestone" } as IMilestone)
    ...
    case ActionTypes.MOVE_MILESTONE: {
      ...
      const prevRank =
        ff[p.index - 1] === undefined ? "" : ff[p.index - 1].rank;
