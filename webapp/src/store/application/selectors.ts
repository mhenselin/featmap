...
export const getWorkspaceByName = (s: IApplication, name: string) =>
  s.workspaces.find((x) => x.name === name);
export const getMembership = (s: IApplication, workspaceId: string) =>
  s.memberships.find((x) => x.workspaceId === workspaceId)!;
export const getSubscription = (s: IApplication, workspaceId: string) =>
