...
export function reducer(state: State = initialState, action: Actions) {
  switch (action.type) {
    case ActionTypes.CREATE_FEATURE: {
      ...
      const featureList = sortFeatures(
        filterFeaturesOnMilestoneAndSubWorkflow(
          state.items,
          feature.milestoneId,
          feature.subWorkflowId
        )
      );
      feature.rank =
        featureList.length === 0
          ? Rank("", "").rank
          : Rank(featureList[featureList.length - 1].rank, "").rank;
      return {
        ...state,
    ...
    case ActionTypes.LOAD_FEATURES: {
      const features = action.payload.map(
        (x) => ({ ...x, kind: "feature" } as IFeature)
