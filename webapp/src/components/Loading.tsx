...
export const Loading: React.FunctionComponent<Readonly<LoadingProps>> = (
  ...
  return (
    <div className="absolute top-1/2 left-1/2 -mt-4 -ml-2 h-8 w-4 text-indigo-700">
      ...
      <div
        className="absolute top-4 h-5 w-4 animate-bounce border-l-[3px] border-gray-200"
        style={{ transformOrigin: "50%, 50%", rotate: "-90deg" }}
      ></div>
      <div
        className="absolute top-4 h-5 w-4 animate-bounce border-r-[3px] border-gray-200"
        style={{ transformOrigin: "50%, 50%", rotate: "90deg" }}
      ></div>
    </div>
  );
